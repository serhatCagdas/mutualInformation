...
for i = 1:length(RL)
    for j = 1:length(NL)
        for k = 1:TN 
            ...
            [data , A]  = generate_bivariate_uniform(mu,Sigma,N);
%             T = table(data);
%             writetable(T,'dataset2.xlsx','Sheet',i,'Range','A1'); 
            X{i,j,k} = data;
            
        end
    end
    Ig(i) = uni_entr( abs(A(1,1)),abs(A(2,1))) + uni_entr( abs(A(1,2)),abs(A(2,2))) - log(abs(det(A)));
end
...
for j = 1:length(NL)
    for i = 1:length(RL) 
        ...
        tic
        for k = 1:TN  
            Icd(i,j,k)   = conditional_dependency(X{i,j,k},2 );
        end
        time_CF(i,j) = toc;
